...
// Some paths inside your WoW installation
...
function parseXmlFile(xmlFileName) {
    ...
    $('Ui > Frame[virtual=true]').toArray().forEach(tag => frameTemplates.push(tag.attribs.name))
}
...
/**
 * Entry point. Read the FrameXML.toc file
 * from the WoW installation directory
 */
...
for (const file of files) {
    log(progress.update(++current, numFiles))
...
log.clear()
console.log(`\nFound ${globalFunctions.length} global functions and ${frameTemplates.length} frame templates, writing to disk...`)

// 3. Exports them as JS scripts
const toHashObject = identifiers => [
    '"use strict;"',
    'module.exports = {',
...
jetpack.write(jetpack.path(__dirname, 'generated', 'luaFunctions.js'), toHashObject(globalFunctions))

frameTemplates.sort()
jetpack.write(jetpack.path(__dirname, 'generated', 'frameTemplates.js'), toHashObject(frameTemplates))
